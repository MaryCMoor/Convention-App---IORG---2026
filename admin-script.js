                ...
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        ...
                        <div style="margin-top: 0.5rem;">
                            ${Array.isArray(user.roles) ? user.roles.map(r => `<span class="role-badge">${escapeHtml(r)}</span>`).join('') : `<span class="role-badge">${escapeHtml(user.role || 'User')}</span>`}
                        </div>
                    </div>
    ...
    container.innerHTML = usersWithEmergency.map(user => {
        ...
        return `
            <div class="emergency-contact-card ${hasMissing ? 'missing' : ''}">
                ...
                <br>
                <span style="color: var(--text-light); font-size: 0.9rem;">
                    📧 ${escapeHtml(user.email)} | ${Array.isArray(user.roles) ? user.roles.map(r => escapeHtml(r)).join(', ') : escapeHtml(user.role)}
                </span>
...
const emergencySearchEl = document.getElementById('emergencySearch');
if (emergencySearchEl) {
    emergencySearchEl.addEventListener('input', (e) => {
        const search = e.target.value.toLowerCase();
